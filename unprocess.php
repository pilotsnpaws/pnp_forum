...
/*  $theta = $lon1 - $lon2; 
  ...
  return ($miles); 
*/
return (  $miles = 60 * rad2deg(acos(sin(deg2rad($lat1)) * sin(deg2rad($lat2)) +  cos(deg2rad($lat1)) * cos(deg2rad($lat2)) * cos(deg2rad($lon1 - $lon2))))); 
}
...
$query =   ' select '
         ...
         . ' phpbb_posts.post_text like "%map:_____-_____%" order by phpbb_posts.post_id desc';
...
if ($fromzip > " " ) {
   ...
   $query = 'select city , state , lat , lon from airports where apt_id = "' . strtoupper($fromzip) . '"';
   $result=mysql_query($query);
   $num = 0;
   if ( $result ) $num=mysql_numrows($result);
   if ($num > 0) { 
...
if ($tozip > " " ) {
   $query = "select city , state , lat , lon from zipcodes where zip = $tozip";
   $result=mysql_query($query);
   $num = 0;
   if ( $result ) $num=mysql_numrows($result);
   if ($num > 0) { 
                   $tocity = mysql_result($result,0,"city");
                   $tostate = mysql_result($result,0,"state");
                   $tolat = mysql_result($result,0,"lat");
                   $tolon = mysql_result($result,0,"lon");
				   }
	   else {
   $query = 'select city , state , lat , lon from airports where apt_id = "' . strtoupper($tozip) . '"';
...
<body>';
echo '
<form action="unprocess.php" method="post">
...
<input type=hidden name=rownum value="';
echo  $rownum ;
echo '"><input type=submit value = "Reload"></form>';
if ( strlen ( $fromzip . $tozip ) == 10 || true ) {
...
'][/map]"></form> ';
echo  $echodist . ' NM<br>[map:' . $fromzip . '-' . $tozip .
']' .mysql_result($messagelist,$rownum,"phpbb_posts.post_id") . '[/map]<br>';

$remecho = '<form action="unprocess.php" method="post">
<input type=hidden name=postid value="' . 
mysql_result($messagelist,$rownum,"phpbb_posts.post_id") . '">
<input type=hidden name=rownum value="' . $rownum . '">
...
$remecho = '<form action="http://www.pilotsnpaws.org/forum/posting.php?mode=edit&f=5&p=' . 
...
mysql_result($messagelist,$rownum,"phpbb_posts.post_id") . '" target = "edit">
Edit Post </a><br>';


}

echo '<a href="http://www.pilotsnpaws.org/forum/posting.php?mode=reply&f=5&t=' . 
mysql_result($messagelist,$rownum,"phpbb_topics.topic_id") . '" target = "reply">
Reply to Post </a>';
