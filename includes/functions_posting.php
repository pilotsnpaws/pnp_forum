...
60 *  degrees(acos(sin(radians(reczip.lat)) * sin(radians(sendzip.lat)) + cos(radians(reczip.lat)) * cos(radians(sendzip.lat)) * cos(radians(reczip.lon - sendzip.lon)))) as dist
 ...
 /* added 2014-07-01 to prevent notifications on trips greater than 1000 miles -- MJG */
( 3959 * acos( cos( radians(reczip.lat) ) 
   * cos( radians( sendzip.lat ) ) 
   * cos( radians(sendzip.lon) - radians(reczip.lon)) + sin(radians(reczip.lat)) 
   * sin( radians(sendzip.lat)))) < 1000 and 
/* end 2014-07-01 change -- MJG */
...
;
		...
		$fromto = " - From " . $row['sendcity'] . ", " . $row['sendstate'] . " To " . $row['reccity'] . ", " . $row['recstate'];
		}
		$db->sql_freeresult($result);
	}
		...
		foreach ($msg_list_ary as $email_template => $email_list)
		{
			foreach ($email_list as $addr)
			{
				...
				$messenger->assign_vars(array(
					...
					'TOPIC_TITLE'	=> htmlspecialchars_decode($topic_title . $fromto),
	...
	switch ($post_mode)
	{
		...
		case 'reply':
			$sql_data[TOPICS_TABLE]['stat'][] = 'topic_last_view_time = ' . $current_time . ',
				...
				topic_bumped = 0, ' . (isset($data['icon_id']) ? 'icon_id = ' . $data['icon_id'] . ',' : '') . '
				topic_bumper = 0' .
