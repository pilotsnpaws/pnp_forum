...
$sql = 'select f.lat as pnp_flat , f.lon as pnp_flon , t.lat as pnp_tlat , t.lon as pnp_tlon from zipcodes f, zipcodes t where f.zip = "' . $pnp_sendZip . '" and t.zip = "' . $pnp_recZip . '"'; 
...
if ( $forum_id == 29 ) {
$pnpres = $db->sql_query('SELECT Concat( "<br>From " , f.city,", ",f.state,"(",f.apt_id,")" ," To ",t.city,", ",t.state,"(",t.apt_id,")") as pnpfttext from airports f,airports t where f.apt_id = "' . $pnp_sendName . '" and t.apt_id = "' . $pnp_recName . '"');
if ($pnpres)
{
$fromtores = $db->sql_fetchrow($pnpres);
$pnpfromtotext = $fromtores['pnpfttext'];
	...
	if (!isset($user_cache[$poster_id]))
	{
		...
		else
		{
			...
			$user_cache[$poster_id] = array(
				...
				'lastvisit' => $row['user_lastvisit'] <> 0 ? $row['user_lastvisit'] : '',
...
for ($i = 0, $end = sizeof($post_list); $i < $end; ++$i)
{
	...
	$postrow = array(
		...
		'POSTER_LASTVISIT' => ($poster_id != ANONYMOUS && !empty($user_cache[$poster_id]['lastvisit'])) ?
$user->format_date($user_cache[$poster_id]['lastvisit']) : '',
...
$template->set_filenames(array(
	'body' => ($view == 'print') ? 'viewtopic_print.html' : 'viewtopic_body2.html')
);
