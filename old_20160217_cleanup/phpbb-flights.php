...
$echodata =  '<?xml version="1.0" encoding="UTF-8"?>
      ...
      <Document>
      <name>Pilots N Paws - Planned Flights</name>
      <description>
      <![CDATA[Pilots N Paws - Planned Flights www.pilotsnpaws.org]]>
      </description>
...
while ($i < $num) {
      ...
      while ($h < min(200,strlen ( $post_text )) ) {
            ...
            $fromapt_id = mysql_result($result,$i,"phpbb_topics.pnp_sendname");
            $toapt_id = mysql_result($result,$i,"phpbb_topics.pnp_recname");
            $fromlat = 0;
            $tolat = 0;
            $fromlon = 0;
            $tolon = 0;
	    $query='select lat , lon from airports where apt_id="' . $fromapt_id . '"';
            if ( $dbg == "yes") $echodata =  $query;
	    ...
	    if ( $dbg == "yes")  $echodata =  mysql_numrows($zresult);
            ...
            $query='select lat , lon from airports where apt_id="' . $toapt_id . '"';
            if ( $dbg == "yes")  $echodata =  ' ' . $query . ' ';
            $zresult=mysql_query($query);
	    if ( $dbg == "yes")  $echodata =  mysql_numrows($zresult);
            if ( mysql_numrows($zresult) > 0 ) {
...
echo $echodata;
            ...
            $echodata =  '<a href="http://pilotsnpaws.org/forum/viewtopic.php?f=29&amp;t=';
fwrite($fh, $echodata);
echo $echodata;
