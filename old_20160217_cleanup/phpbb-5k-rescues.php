...
$query =   ' select '
         ...
         . ' phpbb_topics.pnp_sendzip > " " and '
         . ' phpbb_topics.pnp_reczip > " " '
         . ' order by phpbb_topics.topic_replies limit 500';
...
$echodata =  '<?xml version="1.0" encoding="UTF-8"?>
      ...
      <Document>
      <name>PNP5000 - Transport Requests 9/12/09 - 9/20-09</name>
      <description>
...
while ($i < $num) {
      ...
      while ($h < min(200,strlen ( $post_text )) ) {
	    ...
	    if ( $dbg == "yes")  $echodata =  mysql_numrows($zresult);
            ...
            $color = "blue";
            $age = "2";
            if ( mysql_result($result,$i,"phpbb_topics.topic_replies") == 0 ) $age = "1";
            if ( $dbg == "yes" ) {
               $echodata =  'time=' ;
               $echodata =   time() ; 
               $echodata =  ' topic_time=' ; 
...
echo $echodata;
            ...
            $echodata =  '<a href="http://pilotsnpaws.org/forum/viewtopic.php?f=15&amp;t=';
fwrite($fh, $echodata);
echo $echodata;
