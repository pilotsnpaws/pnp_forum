...
if ( $forum_id == 5 )
{
$pnpres = $db->sql_query('SELECT Concat( "<br>From " , f.city,", ",f.state,"(",f.zip,")" ," To ",t.city,", ",t.state,"(",t.zip,")") as pnpfttext from zipcodes f,zipcodes t where f.zip = "' . $row['pnp_sendZip'] . '" and t.zip = "' . $row['pnp_recZip'] . '"');
if ($pnpres)
{
$fromtores = $db->sql_fetchrow($pnpres);
$pnpfromtotext = $fromtores['pnpfttext'];
$topic_title .= $pnpfromtotext;
$db->sql_freeresult($pnpres);
}
}
if ( $forum_id == 29 ) {

$pnpres = $db->sql_query('SELECT Concat( "<br>Space available From " , f.city,", ",f.state,"(",f.apt_id,")" ," To ",t.city,", ",t.state,"(",t.apt_id,")") as pnpfttext from airports f,airports t where f.apt_id = "' . $row['pnp_sendName'] . '" and t.apt_id = "' . $row['pnp_recName'] . '"');
if ($pnpres)
{
$fromtores = $db->sql_fetchrow($pnpres);
$pnpfromtotext = $fromtores['pnpfttext'];
