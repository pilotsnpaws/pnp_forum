...
// mike changed to 360, 6 mins. the cron from dreamhost runs the queue every 5 mins. so this should be safe. 
// setting this lower will cause duplicate emails, as php is sending but the new process will think it died and start again
//	mail('nekbet@gmail.com','cron check',$config['cron_lock']);
//	mail('nekbet@gmail.com','queue last interval math',time() - $config['queue_interval']);
...
switch ($cron_type)
{
	case 'queue':
		...
		logEvent('cron.php running $queue->process. Queue size: ' . $queue_formatted_before . ' bytes');
	...
	case 'prune_forum':
		//added by mike
		//mail('nekbet@gmail.com','prune forum','this is start of prune');
...
// mail ('nekbet@gmail.com','test closing db','this runs before unlock cron and garbace collection');
	unlock_cron();
	garbage_collection();
